...
public class FavoriteFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        dataAdapter = new DataAdapter(new FavoriteService(getActivity()).getAll());
        ...
        private void showPopupMenu(View view, final Data data, final int position) {
            ...
            popup.setOnMenuItemClickListener(menuItem -> {
                if(menuItem.getItemId() == R.id.item_delete_from_favorite) {
                    ...
                    new FavoriteService(getActivity()).deleteById(data.getId());

                    switch (data.getKind()) {
                        case "book":
                            new BookService(getActivity()).save(data);
                            break;

                        case "movie":
                            new MovieService(getActivity()).save(data);
                            break;

                        case "podcast":
                            new PodcastService(getActivity()).save(data);
                            break;
                    }
                }
                return false;
